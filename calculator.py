...
def get_antenna_gain(space_antenna_diameter, ground_antenna_diameter, signal_frequency, antenna_efficiency):
    wavelength = c / signal_frequency
    transmitter_gain = 20 * np.log10(space_antenna_diameter) + 20 * np.log10(signal_frequency/1E9) + 17.8
    receiver_gain = ((np.pi ** 2) * (ground_antenna_diameter ** 2))/(wavelength ** 2) * antenna_efficiency
    receiver_gain_db = 10 * np.log10(receiver_gain / 1)
    return transmitter_gain, receiver_gain_db
...
def get_required_data_rate_db(generated_data_rate, duty_cycle, downlink_time_ratio):
    ...
    required_data_rate_db = 10 * np.log10(1 / required_data_rate_unit)
...
def get_system_noise_temperature_db(system_noise_temperature):
    return 10 * np.log10(1 / system_noise_temperature)


def get_received_snr(data):
    transmitter_power_db = get_transmitter_power_db(data.tx_power)
    transmitter_gain, receiver_gain = get_antenna_gain(data.space_antenna_diameter, data.ground_antenna_diameter, data.signal_frequency, data.antenna_efficiency)
    ...
    if data.generated_data_rate_given > 0.5:
    ...
    if data.system_noise_temp_given > 0.5:
    ...
    # return transmitter_power_db * data.tx_loss_factor * transmitter_gain * data.tx_path_loss * receiver_gain * space_loss * pointing_loss * \
    #       data.rx_loss_factor * required_data_rate * 1.38e-23 * system_noise_temperature
    return transmitter_power_db + data.tx_loss_factor + transmitter_gain + data.tx_path_loss + receiver_gain + space_loss + pointing_loss + \
           data.rx_loss_factor + required_data_rate + system_noise_temperature + 228.6
